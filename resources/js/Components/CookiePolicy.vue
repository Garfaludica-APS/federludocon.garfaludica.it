...
<template>
	<div class="flex flex-col ml:flex-row items-center fixed transition-[bottom] duration-500 inset-x-4 bg-zinc-200 dark:bg-zinc-900 text-gray-900 dark:text-gray-100 z-50 p-8 rounded-xl shadow-2xl ring-4 ring-gray-400 dark:ring-gray-950 text-sm ml:text-base" :class="{'bottom-4': visible, '!bottom-[-100%]': !visible}">
		<div class="grow">
			...
			<Link href="#" class="inline-block mt-4 text-sm text-green-600 dark:text-green-500 underline">{{ $t('Privacy Policy') }}</Link>
		</div>
		<div class="flex-none flex flex-col sm:flex-row ml:flex-col justify-center items-center max-w-full ml:max-w-xs mt-10 ml:mt-0 ml:ml-20">
			...
			<p class="mt-4 sm:mt-0 ml:mt-4 sm:pl-8 ml:pl-0 text-xs ml:text-sm text-stone-700 dark:text-stone-500 text-center leading-tight" v-html="$t('cookie_no_decline_text')"></p>
		</div>
	</div>
</template>
