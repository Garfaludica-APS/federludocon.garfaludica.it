...
const hotelMenuOpen = ref(null);

function isHotelRoute(hotel, routeName = undefined)
{
	const params = route().params;
	const isHotel = params.hotel == hotel.id;
	if (!isHotel)
		return false;
...
onMounted(() => {
	for (var i = 0; i < page.props.auth.admin.hotels.length; i++) {
		var h = page.props.auth.admin.hotels[i];
		if (isHotelRoute(h)) {
			hotelMenuOpen.value = h.name;
			break;
		}
	}
});
		...
		<aside class="lg:py-2 lg:pl-2 w-72 fixed flex z-40 top-0 h-screen transition-[left]" :class="{ '-left-72 lg:left-0': !isMenuOpen, 'left-0': isMenuOpen }">
			<nav class="lg:rounded-2xl flex-1 flex flex-col overflow-hidden text-slate-100 bg-slate-900">
				...
				<ul class="flex-1 overflow-y-auto overflow-x-hidden">
					...
					<li>
						<AsideNavLink :href="route('admin.bookings.index')" :active="route().current().startsWith('admin.bookings')">
							<template #icon>
								<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="inline-block w-16 min-w-16 h-6">
					...
					<li v-for="hotel in page.props.auth.admin.hotels">
						<AsideNavLink @click="hotelMenuOpen === hotel.name ? hotelMenuOpen = null : hotelMenuOpen = hotel.name" :active="isHotelRoute(hotel)">
							<template #icon>
							<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="inline-block w-16 min-w-16 h-6">
						...
						<MazTransitionExpand>
							<ul v-if="hotelMenuOpen === hotel.name" class="bg-slate-700">
								<li>
									<AsideChildNavLink :href="route('admin.hotel.presentation.edit', { hotel: hotel })" :active="isHotelRoute(hotel, 'admin.hotel.presentation')">
										{{ $t('Presentation Page') }}
									</AsideChildNavLink>
								</li>
								<li>
									<AsideChildNavLink :href="route('admin.hotel.rooms.index', { hotel: hotel })" :active="isHotelRoute(hotel, 'admin.hotel.rooms')">
										{{ $t('Manage Rooms') }}
									</AsideChildNavLink>
								</li>
								<li>
									<AsideChildNavLink :href="route('admin.hotel.meals.index', { hotel: hotel })" :active="isHotelRoute(hotel, 'admin.hotel.meals')">
										{{ $t('Manage Meals') }}
									</AsideChildNavLink>
								</li>
								<li>
									<AsideChildNavLink :href="route('admin.hotel.externalBookings.index', { hotel: hotel })" :active="isHotelRoute(hotel, 'admin.hotel.externalBookings')">
										{{ $t('External Bookings') }}
									</AsideChildNavLink>
								</li>
							</ul>
